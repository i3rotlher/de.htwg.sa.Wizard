...
import de.htwg.sa.wizard.aview.gui
...
class SwingGUI(controller: ControllerInteface) extends Frame {
  ...
  def wizard_trump(): Unit = {
    ...
    wish_trump_panel = new FlowPanel {
      ...
      var trump = gui.Card_panel(controller.getGamestate().trump_Card)
      val hand_label = new Label("Your cards:")
      var hand = new Hand_panel(controller.getGamestate().players(controller.active_player_idx()).hand)
