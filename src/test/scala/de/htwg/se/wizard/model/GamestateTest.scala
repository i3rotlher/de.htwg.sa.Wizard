  ...
  "Gamestate" when {
    "initialized with these players" should {
      ...
      "and when handing out the cards in round 6 they should all be different" in {
        ...
        (game0.players.head.getHand diff game0.players(1).getHand).size shouldBe 6
        (game0.players(1).getHand diff game0.players(2).getHand).size shouldBe 6
      }
      "and generate a trumpcard out of the remaining cards" in {
        ...
        game1.players.head.getHand.contains(game1.trump_Card) shouldBe false
        game1.players(1).getHand.contains(game1.trump_Card) shouldBe false
        game1.players(2).getHand.contains(game1.trump_Card) shouldBe false
      ...
      "and when the trumpcard is generated in the last round" in {
        ...
        (game8.players.head.getHand diff game8.players(1).getHand).size shouldBe 20
        (game8.players(1).getHand diff game8.players(2).getHand).size shouldBe 20
      ...
      "When a player played a card should have the remaining cards" in {
        ...
        val to_play = game6.players.head.getHand.head
        game6 = game6.playCard(to_play, game6.players.head)
        game6.players.head.getHand.contains(to_play) shouldBe false
      }
